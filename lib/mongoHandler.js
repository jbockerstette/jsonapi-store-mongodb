...
//This value must be less then the default timeout time of 120000 ms.
var KEEP_ALIVE_TIMER_MS = 60000;
...
/**
 * We must fire off this method to keep from getting timedout. Getting timed out is bad because
 * I have no idea how to prevent dead TCP connections which will eventually crash the system.
 */
MongoStore.prototype.keepAlive = function () {
  var self = this;
  self._db.collection("KeepAlive").find({ _id: 0 }).toArray(function (err, tags) {
...
MongoStore.prototype.search = function(request, callback) {
  ...
  async.parallel({
    ...
    totalRows: function(asyncCallback) {
      return collection.find(criteria).count(asyncCallback);
    }
